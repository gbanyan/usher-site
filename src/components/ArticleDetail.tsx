import Image from "next/image";
import Link from "next/link";
import { getArticleSchema } from "@/lib/jsonld";
import { formatDate } from "@/lib/utils";
import JsonLd from "@/components/JsonLd";
import PageHeader from "@/components/PageHeader";
import MarkdownRenderer from "@/components/MarkdownRenderer";
import ArticleCard from "@/components/ArticleCard";
import ArticleAttachments from "@/components/ArticleAttachments";
import {
  ARTICLE_SECTION_CONFIG,
  type ArticleSectionType,
} from "@/lib/article-sections";
import type { Article, ArticleSummary } from "@/lib/types";

interface ArticleDetailProps {
  type: ArticleSectionType;
  slug: string;
  article: Article;
  related: ArticleSummary[];
}

/**
 * Shared detail-page renderer for blog, notice, related-news and the
 * blog-derived story/guides sections. The five per-route pages only keep
 * generateStaticParams/generateMetadata and delegate the markup here.
 */
export default function ArticleDetail({
  type,
  slug,
  article,
  related,
}: ArticleDetailProps) {
  const config = ARTICLE_SECTION_CONFIG[type];

  // story/guides only link to articles in the same category; the other
  // sections keep the API-provided related set (which can span content types,
  // so ArticleCard resolves each link from the article's own content_type).
  const relatedArticles = config.categorySlug
    ? related.filter((item) =>
        (item.categories ?? []).some(
          (category) => category.slug === config.categorySlug
        )
      )
    : related;

  return (
    <>
      <JsonLd
        data={getArticleSchema(
          article,
          `${config.path}/${slug}`,
          config.fallbackDescription
        )}
      />
      <div data-pagefind-body>
        <PageHeader
          title={article.title}
          items={[
            { label: config.label, href: config.path },
            { label: article.title },
          ]}
          description={
            article.published_at
              ? formatDate(article.published_at)
              : undefined
          }
        />

        <article className="mx-auto max-w-4xl px-6 py-12 lg:px-8">
          <div className="mb-8">
            <div className="flex flex-wrap items-center gap-4 text-sm text-white/70">
              {article.author_name && (
                <span>
                  <span className="sr-only">作者：</span>
                  {article.author_name}
                </span>
              )}
            </div>

            {article.categories?.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {article.categories.map((category) => (
                  <span
                    key={category.id}
                    className="inline-flex items-center rounded-full bg-accent/15 px-3 py-1 text-xs font-medium text-accent"
                  >
                    {category.name}
                  </span>
                ))}
              </div>
            )}

            {article.tags?.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {article.tags.map((tag) => (
                  <span
                    key={tag.slug}
                    className="inline-flex items-center rounded-full border border-white/20 px-3 py-1 text-xs text-white/80"
                  >
                    {tag.name}
                  </span>
                ))}
              </div>
            )}
          </div>

          {article.featured_image_url && (
            <figure className="mb-8 overflow-hidden rounded-lg">
              <Image
                src={article.featured_image_url}
                alt={article.featured_image_alt || article.title}
                width={1200}
                height={630}
                className="h-auto w-full object-cover"
                sizes="(max-width: 896px) 100vw, 896px"
              />
            </figure>
          )}

          <div className="prose prose-lg max-w-none">
            <MarkdownRenderer content={article.content} />
          </div>

          <ArticleAttachments articleSlug={slug} attachments={article.attachments} />

          {relatedArticles.length > 0 && (
            <section
              className="mt-12 border-t border-white/10 pt-8"
              aria-labelledby="related-heading"
              data-pagefind-ignore
            >
              <h2
                id="related-heading"
                className="text-xl font-semibold text-white"
              >
                {config.relatedHeading}
              </h2>
              <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {relatedArticles.map((relatedArticle) => (
                  <ArticleCard
                    key={relatedArticle.id}
                    article={relatedArticle}
                    basePath={config.categorySlug ? config.path : undefined}
                  />
                ))}
              </div>
            </section>
          )}
        </article>

        <nav className="mx-auto max-w-4xl px-6 pb-12 lg:px-8" data-pagefind-ignore>
          <Link
            href={config.path}
            className="inline-flex items-center text-sm font-medium text-accent transition-colors hover:text-accent-light"
          >
            <span aria-hidden="true">&larr;</span>
            <span className="ml-2">返回{config.label}列表</span>
          </Link>
        </nav>
      </div>
    </>
  );
}
